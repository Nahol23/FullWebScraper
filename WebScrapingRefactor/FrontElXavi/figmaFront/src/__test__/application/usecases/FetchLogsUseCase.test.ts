...
describe('FetchLogsUseCase', () => {
  ...
  const mockLogs: ExecutionHistory[] = [
    { id: 'log1', timestamp: new Date().toISOString(), status: 200, duration: 100 },
    { id: 'log2', timestamp: new Date().toISOString(), status: 404, duration: 50, errorMessage: 'Not found' },
  ];
  ...
  it('dovrebbe restituire i log per una configurazione', async () => {
    ...
    expect(vi.mocked(mockRepo.getLogsByConfig)).toHaveBeenCalledWith('cfg123', undefined);
  ...
  it('dovrebbe passare il limite al repository', async () => {
    ...
    expect(vi.mocked(mockRepo.getLogsByConfig)).toHaveBeenCalledWith('cfg123', 5);
  });
});