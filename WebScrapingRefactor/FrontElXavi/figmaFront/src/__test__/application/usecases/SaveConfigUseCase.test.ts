...
describe('SaveConfigUseCase', () => {
  ...
  beforeEach(() => {
    mockRepo = {
      save: vi.fn().mockImplementation((config) => Promise.resolve({ ...config, id: '123' })),
      getAll: vi.fn(),
      getById: vi.fn(),
      update: vi.fn(),
      delete: vi.fn(),
  ...
  it('dovrebbe salvare una configurazione valida', async () => {
    ...
    expect(vi.mocked(mockRepo.save)).toHaveBeenCalledWith(expect.objectContaining(validConfig));
  ...
  it('dovrebbe lanciare ValidationError se il nome è vuoto', async () => {
    ...
    await expect(useCase.execute(invalidConfig)).rejects.toThrow(ValidationError);
