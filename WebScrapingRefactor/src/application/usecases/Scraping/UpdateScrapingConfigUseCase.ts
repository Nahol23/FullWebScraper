...
export class UpdateScrapingConfigUseCase {
  ...
  async execute(id: string, updates: Partial<ScrapingConfig>): Promise<ScrapingConfig> {
    const existing = await this.scrapingConfigRepository.getById(id);
    if (!existing) {
      throw new ConfigNotFoundError(id, "Scraping configuration");
    }
    ...
    await this.scrapingConfigRepository.update(id, updatedConfig);
